...
RUN pnpm --filter=hey-api-test-app exec playwright install --with-deps
RUN pnpm run build
...
RUN --mount=type=cache,id=pnpm,target=/pnpm/store pnpm install --frozen-lockfile
RUN pnpm run build
RUN pnpm deploy --filter=hey-api-test-server --prod /prod/server
RUN pnpm deploy --filter=hey-api-test-app --prod /prod/app
